...
class SearchPhp_AdminController extends Pimcore_Controller_Action_Admin
{
    ...
    public function settingsAction()
    {
        ...
        $this->view->translate = new Zend_Translate('csv', PIMCORE_PLUGINS_PATH .
            SearchPhp_Plugin::getTranslationFile($language), $language, array('delimiter' => ','));
    ...
    public function getStateAction()
    {
        ...
        if (SearchPhp_Plugin::frontendCrawlerRunning() or
            SearchPhp_Plugin::frontendCrawlerScheduledForStart() or
            !SearchPhp_Plugin::frontendConfigComplete()
        ) {
            $frontendButtonDisabled = true;
        }

        $message = str_replace("------------------------------------------- ", "<br/>",
            SearchPhp_Plugin::getPluginState());

        $frontendStopButtonDIsabled = false;
        if (!SearchPhp_Plugin::frontendConfigComplete() or
            !SearchPhp_Plugin::frontendCrawlerRunning() or
            SearchPhp_Plugin::frontendCrawlerStopLocked()
        ) {
    ...
    public function setConfigAction()
    {
        ...
        if (is_numeric($values["search.frontend.crawler.maxThreads"])) {
            $this->config["search"]["frontend"]["crawler"]["maxThreads"] =
                $values["search.frontend.crawler.maxThreads"];
        }

        if (is_numeric($values["search.frontend.crawler.maxLinkDepth"])) {
            $this->config["search"]["frontend"]["crawler"]["maxLinkDepth"] =
                $values["search.frontend.crawler.maxLinkDepth"];
        } else {
            $this->config["search"]["frontend"]["crawler"]["maxLinkDepth"] = 15;
        }

        $this->config["search"]["frontend"]["categories"] =
            base64_encode($values["search.frontend.categories"]);
        $this->config["search"]["frontend"]["urls"] =
            base64_encode($values["search.frontend.urls"]);
        $this->config["search"]["frontend"]["validLinkRegexes"] =
            base64_encode($values["search.frontend.validLinkRegexes"]);
        $this->config["search"]["frontend"]["invalidLinkRegexesEditable"] =
            base64_encode($values["search.frontend.invalidLinkRegexesEditable"]);
        $this->config["search"]["frontend"]["crawler"]["contentStartIndicator"] =
            base64_encode($values["search.frontend.crawler.contentStartIndicator"]);
        $this->config["search"]["frontend"]["crawler"]["contentEndIndicator"] =
            base64_encode($values["search.frontend.crawler.contentEndIndicator"]);
        try {
