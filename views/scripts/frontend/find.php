...
<?php if (!$this->omitSearchForm) { ?>
    ...
    <?php if (!$this->omitJsIncludes) { ?>
        ...
        <link rel="stylesheet" href="/plugins/SearchPhp/static/css/jquery-autocomplete.css"
              type="text/css"/>
        <script type="text/javascript"
                src="/plugins/SearchPhp/static/js/frontend/jquery.autocomplete.js"></script>
        ...
        <form method="post" action="?search=true" id="searchForm">
            ...
            <?php if (is_array($this->availableCategories) and count($this->availableCategories) > 0) { ?>
                ...
                <select id="searchCat" name="cat">
                    <option
                        value=""><?php echo $this->translate('search_all_categories') ?></option>
                    <?php foreach ($this->availableCategories as $category) { ?>
                        <option
                            <?php if ($this->category == $category){ ?>selected="selected"<?php } ?>
                            value="<?php echo $category ?>"><?php echo $this->translate('search_category_' . $category) ?></option>
                    <?php } ?>
                </select>
            <?php } ?>
            <span class="submit_wrapper"><input class="submit" type="submit"
                                                value="<?php echo $this->translate('search_submit') ?>"/></span>
...
<div id="search_info">
    ...
    if (count($this->searchResults) >= 1) {
        ...
        ?>
        <?php echo $this->translate('search_showing_results') ?> <?php echo $start + 1 ?> - <?php echo $end ?> <?php echo $this->translate('search_results_of') ?> <?php echo $this->total ?>
        <br/>
    <?php } else {
...
<?php if (!empty($this->suggestions)) { ?>
    ...
    <?php if (count($this->suggestions) > 5) { ?>
        ...
        <a style="cursor:pointer;" id="search_result_additional_suggestions_hint"
           onclick="$('#search_result_additional_suggestions_hint').hide();$('#search_result_additional_suggestions').show()"><?php echo (count($this->suggestions) - 5) . ' ' . $this->translate('more_search_suggestions') ?></a>
    <?php } ?>
<?php } ?>
            ...
            <div class="search_result_category category_<?php echo $key ?>">
                ...
                <?php foreach ($categoryResults as $searchResult) { ?>
                    ...
                    <div class="search_result <?php if (is_array($searchResult['categories'])) {
                        ...
                        <a href="<?php echo $searchResult['url'] ?>"><?php if (!empty($searchResult['title']) and trim($searchResult['title']) != "") {
                                echo trim($searchResult['title']);
                            } else {
                                echo $searchResult['url'];
                            } ?></a><br/>
                        <?php if ($searchResult['h1']) { ?>
                            <strong><?php echo $searchResult['h1'] ?></strong>  <?php } ?>
...
<?php if (count($this->searchResults) > 0) { ?>
    <div id="search_paging">
        ...
        <?php } ?>
        <?php if ($this->page > 1) { ?>
            <a href="?query=<?php echo $this->query ?>&cat=<?php echo $this->category ?>&page=<?php echo $this->page - 1 ?>">
                &lt;</a>
        <?php } ?>
        <?php for ($i = $pageStart; $i <= $pageEnd; $i++) { ?>
            <a <?php if ($this->page == $i) { ?>class="active"<?php } ?>
               href="?query=<?php echo $this->query ?>&cat=<?php echo $this->category ?>&page=<?php echo $i ?>"><?php echo $i ?></a>
        <?php } ?>
